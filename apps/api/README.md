...
Google Cloud console: create one OAuth client per platform (Web application with `http://localhost:5173` as authorized origin; iOS with bundle id `com.head.board`; Android with package `com.head.board` and the signing certificate SHA-1). Apple Developer portal: enable Sign in with Apple on the App ID, create a Services ID for the web with an https return URL (Apple does not accept `localhost`).
...
| `Auth:AppleClientIds` | `[]` | Accepted audiences for Apple id-tokens: the Services ID (web) and the iOS bundle id (`com.head.board`). Legacy `Auth:AppleClientId` is still honoured. |
