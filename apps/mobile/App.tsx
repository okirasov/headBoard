...
function useDevAutologin() {
  ...
  useEffect(() => {
    ...
    const v = process.env.EXPO_PUBLIC_DEV_VIEW; if (v === 'board' || v === 'review' || v === 'digest' || v === 'calendar' || v === 'archive') st.set({ mView: v });
...
function Root() {
  ...
  const titles = { board: T.board, review: T.resurface, digest: T.digTitle, calendar: T.calendar, archive: T.archiveTitle };
  const subs = { board: phrases.openTasks(openN, lang), review: phrases.forgottenN(stats.staleN, lang), digest: todayLabel(now, lang), calendar: T.gcal, archive: phrases.archivedN(tasks.filter(x => x.status === 'archived').length, lang) };
  return (
