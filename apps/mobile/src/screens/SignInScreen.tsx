...
export function SignInScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.bg, paddingTop: 96, paddingHorizontal: 22, paddingBottom: 44 }}>
      ...
      <View style={{ flex: 1 }} />
      <View style={{ gap: 10 }}>
        <Pressable onPress={() => signIn('Google')} style={{ height: 52, borderRadius: 14, borderWidth: 1, borderColor: t.lineStrong, backgroundColor: t.card, alignItems: 'center', justifyContent: 'center' }}>
          <Text style={txt(14.5, { w: 600, color: t.ink })}>{T.google}</Text>
        </Pressable>
        <Pressable onPress={() => signIn('Apple')} style={{ height: 52, borderRadius: 14, backgroundColor: t.ink, alignItems: 'center', justifyContent: 'center' }}>
