    ...
    <div className="absolute inset-0 z-[70] flex items-center justify-center bg-bg p-40">
      <div className="flex w-432 flex-col gap-22 rounded-20 border border-line bg-panel p-34 shadow-auth">
        ...
        <div className="flex flex-col gap-9">
          <button type="button" onClick={() => void signInWith('Google')} className="flex h-46 cursor-pointer items-center justify-center rounded-12 border border-lineStrong bg-card text-14 font-semibold hover:border-acc hover:text-acc">{T.google}</button>
          <button type="button" onClick={() => void signInWith('Apple')} className="flex h-46 cursor-pointer items-center justify-center rounded-12 bg-ink text-14 font-semibold text-onInk hover:bg-inkHov">{T.apple}</button>
        </div>
